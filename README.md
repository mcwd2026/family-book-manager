...
- **成员管理**：每位家庭成员一个档案，类型为 `大人` / `小孩`；小孩记录出生日期，自动计算年龄（如"6岁11个月"）与适合其阅读的藏书数
- **阅读记录**：每次阅读绑定成员 + 书目，支持评分、日期、备注，可取消（应用内弹窗二次确认）
- **首页统计**：总藏书数、本月阅读总数，以及按成员拆分的本月/累计阅读数
- **智能推荐**：基于成员生日计算实际年龄，按书目的 `ageMin/ageMax` 区间匹配，不按成员类型过滤
...
> 默认账号**仅在数据库为空（首次启动）时**由种子脚本创建。之后修改密码请登录后在「我的 → 修改密码」中操作；已有数据的环境变量 `ADMIN_PASSWORD` 变更不会覆盖现有密码。
...
- **忘记密码**：进容器重置，例如
  ...
  # 在容器内用 node + bcryptjs 生成新哈希后更新 users 表，或删除 data/dev.db 重新初始化（会清空数据）
  ```
